...
/**
 * Inspired by WordPress sanitize functions.
 *
 * Examples:
 *
 *      $validations = [
 ...
 *      $sanitize = ['alias'];
 *
 *      $validate = new Sanitizer(new \TriTan\Common\Hooks\ActionFilterHook(), $validations, $required, $sanitize);
 *      if($validate->validateItems($_POST))
 *      {
 *          $_POST = $validate->items($_POST);
 *          // now do what you need, $_POST has been sanitized.
 *      }
 *
 *      Validate one item:
 *      $validate = new Sanitizer()->validateItem('email@gmail.', 'email');
 *
 *      Sanitize one item:
 *      $sanitize = new Sanitizer()->item('<b>word</b>', 'string');
 */

final class Sanitizer implements \TriTan\Interfaces\SanitizerInterface
{
    public $regexes = [
    ...
    public function items(array $items, $context = 'save')
    {
        ...
        foreach ($items as $key => $val) {
            ...
            $items[$key] = $this->validateItem($val, $this->validate[$key]);
        }
     ...
     */
    protected function addError($field, $type = 'string')
    {
