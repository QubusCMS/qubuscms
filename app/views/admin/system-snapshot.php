    ...
    <section class="content">
        ...
        <div class="box box-default">
            <pre>
                ...
                $report .= ActionFilterHook::getInstance()->{'applyFilter'}('system_snapshot_report_before', '');

                $report .= "\n\t" . esc_html__('** Qubus CMS DATA **') . PHP_EOL . PHP_EOL;
                $report .= esc_html__('Site URL:') . "\t\t\t\t\t\t" . site_url() . PHP_EOL;
                $report .= esc_html__('Qubus CMS Release:') . "\t\t\t\t\t" . CURRENT_RELEASE . PHP_EOL;
                ...
                $report .= "\n\t" . esc_html__('** Qubus CMS CONFIG **') . PHP_EOL . PHP_EOL;
                ...
                $report .= esc_html__('Database Version:') . "\t\t\t\t\t" . $this->dbversion . PHP_EOL;
                ...
                $report .= esc_html__('Cookie Path:') . "\t\t\t\t\t\t" . ((new \TriTan\Common\FileSystem(ActionFilterHook::getInstance()))->{'isWritable'}($this->app->config('cookies.savepath')) ? '<font color="green">' . $this->app->config('cookies.savepath') . '</font>' : '<font color="red">' . $this->app->config('cookies.savepath') . '</font>') . PHP_EOL;
                $report .= esc_html__('Regular Cookie TTL:') . "\t\t\t\t\t" . convert_seconds_to_time($this->app->config('cookies.lifetime')) . PHP_EOL;
                $report .= esc_html__('Secure Cookie TTL:') . "\t\t\t\t\t" . convert_seconds_to_time($option->{'read'}('cookieexpire')) . PHP_EOL;
                $report .= esc_html__('File Save Path:') . "\t\t\t\t\t\t" . ((new \TriTan\Common\FileSystem(ActionFilterHook::getInstance()))->{'isWritable'}(Container::getInstance()->{'get'}('site_path') . 'files' . DS) ? '<font color="green">' . Container::getInstance()->{'get'}('site_path') . 'files' . DS . '</font>' : '<font color="red">' . Container::getInstance()->{'get'}('site_path') . 'files' . DS . '</font>') . PHP_EOL;
                $report .= esc_html__('Qubus CMS Node:') . "\t\t\t\t\t\t" . ((new \TriTan\Common\FileSystem(ActionFilterHook::getInstance()))->{'isWritable'}(TTCMS_NODEQ_PATH) ? '<font color="green">' . TTCMS_NODEQ_PATH . '</font>' : '<font color="red">' . TTCMS_NODEQ_PATH . '</font>') . PHP_EOL;
                ...
                $report .= ActionFilterHook::getInstance()->{'applyFilter'}('system_snapshot_report_after', '');
                // end it all
                $report .= PHP_EOL;
