...
$app->group('/admin', function () use ($app, $opt, $qudb, $current_user) {

    /**
     * Before route check.
     */
    $app->before('GET|POST', '/user/profile/', function () use ($app) {
        if (!is_user_logged_in()) {
            ttcms()->obj['flash']->{'error'}(
        ...
        if (isset($app->req->post['__ttcmsnonce']) && !Nonce::verify($app->req->post['__ttcmsnonce'], 'my-profile')) {
            ttcms()->obj['flash']->{'error'}(
     ...
     * Before route check.
     */
    $app->before('GET|POST', '/user/create/', function () use ($app) {
        ...
        if (isset($app->req->post['__ttcmsnonce']) && !Nonce::verify($app->req->post['__ttcmsnonce'], 'create-user')) {
            ttcms()->obj['flash']->{'error'}(
                esc_html__('Invalid or missing CSRF token.'),
                $app->req->server['HTTP_REFERER']
            );
    ...
    $app->match('GET|POST', '/user/create/', function () use ($app, $current_user) {
        if ($app->req->isPost()) {
            ...
            if (strlen($password) < ActionFilterHook::getInstance()->{'applyFilter'}('password_length', 6)) {
                ttcms()->obj['flash']->{'error'}(
                    t__('Password cannot be less than 6 characters.', 'tritan-cms'),
                    $app->req->server['HTTP_REFERER']
                );
                exit();
            }
            ...
            if (!validate_email($app->req->post['user_email'])) {
                (new TriTan\Common\Uri(ActionFilterHook::getInstance()))->{'redirect'}($app->req->server['HTTP_REFERER']);
                exit();
            }
            ...
            try {
                ...
                if ($update) {
                    $user_id = ttcms_update_user($array_merge);
                } else {
                    $user_id = ttcms_insert_user($array_merge);
                }
                /**
                 * Set user's role.
                 */
                ...
                if ($app->req->post['sendemail'] == '1') {
                    queue_new_user_email((int) $user_id, $password);
                }
     ...
     * Before route check.
     */
    $app->before('GET|POST', '/user/(\d+)/', function ($id) use ($app) {
        ...
        if (isset($app->req->post['__ttcmsnonce']) && !Nonce::verify($app->req->post['__ttcmsnonce'], "update-user-{$id}")) {
            ttcms()->obj['flash']->{'error'}(
                esc_html__('Invalid or missing CSRF token.'),
                $app->req->server['HTTP_REFERER']
            );
    ...
    $app->match('GET|POST', '/user/(\d+)/', function ($id) use ($app, $current_user) {
        if ($app->req->isPost()) {
             ...
             *            User's id.
             */
            ActionFilterHook::getInstance()->{'doAction'}('pre_update_user', (int) $id);
    ...
    $app->get('/user/(\d+)/switch-back/', function ($id) use ($app, $opt) {
        ...
        try {
            ...
            Cascade::getLogger('error')->{'error'}(
                ...
                [
                    'User Router' => '/user/(\d+)/switch-back/ #1'
                ]
            );
        }
        ...
        try {
            ...
            Cascade::getLogger('error')->{'error'}(
                ...
                [
                    'User Router' => '/user/(\d+)/switch-back/ #2'
                ]
            );
        }
    ...
    $app->match('GET|POST', '/user/lookup/', function () use ($app, $qudb) {
        $user = $qudb->getRow(
            $qudb->prepare("SELECT user_fname,user_lname,user_email FROM {$qudb->base_prefix}user WHERE user_id = ?", [$app->req->post['user_id']])
        );
