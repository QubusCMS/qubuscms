...
class Collection
{
        ...
        if (!file_exists($this->filepath)) {
            ...
            if (is_null($data)) {
                throw new InvalidJsonException(
                    "Failed to load data. File '{$this->filepath}' contain invalid JSON format."
                );
            }
        }
        ...
        if ($this->isModeTransaction()) {
            ...
            if (!is_dir($dir)) {
                throw new DirectoryNotFoundException(
                    "Cannot save database. Directory {$dir} not found or it is not directory."
                );
            }
    ...
    protected function autoIncrementIdPrefix()
    {
        ...
        $first_key = preg_replace('/^.*?(nodeq)/', '', $this->filepath);
        ...
        return str_replace($qudb->prefix, '', $key);
    }
