...
abstract class DataLayer
{
    ...
    /** @var object $qudb database object */
    private $qudb;
     ...
     */
    public function __construct(string $entity, array $required, string $primary = 'id', bool $timestamps = false)
    {
        $this->entity = $entity;
    ...
    {
        try {
            $stmt = app()->qudb->getPdo()->prepare($this->statement . $this->group . $this->order . $this->limit . $this->offset);
    ...
    {
        $stmt = app()->qudb->getPdo()->prepare($this->statement);
        $stmt->execute($this->params);
