                    ...
                    <tbody>
                        <?php foreach ($this->users as $user) : $data = get_userdata($user['user_id']); ?>
                            <tr class="gradeX">
                                ...
                                <td class="text-center">
                                    <?php if ((int) $user['user_id'] === (int) get_current_user_id()) : ?>
                                    <a href="<?= admin_url('user/profile/'); ?>" data-toggle="tooltip" data-placement="top" title="<?= esc_attr__('Update'); ?>"><button type="button" class="btn bg-yellow"><i class="fa fa-edit"></i></button></a>
                                    <?php else : ?>
                                    ...
                                    <a href="<?= admin_url('user/' . (int) $user['user_id'] . '/reset-password/'); ?>" data-toggle="tooltip" data-placement="top" title="<?= esc_attr__('Reset Password'); ?>"><button type="button" class="btn bg-purple"><i class="fa fa-redo"></i></button></a>
                                    <?php if (!isset($this->app->req->cookie['SWITCH_USERBACK']) && (int) $user['user_id'] !== (int) $this->current_user_id) : ?>
                                        <a<?= ae('switch_user'); ?> href="<?= admin_url('user/' . (int) esc_html($user['user_id']) . '/switch-to/'); ?>" data-toggle="tooltip" data-placement="top" title="<?= esc_attr__('Switch to'); ?>"><button type="button" class="btn bg-blue"><i class="fa fa-exchange-alt"></i></button></a>
                                    <?php endif; ?>
                                    <?php if ((int) $user['user_id'] !== (int) 1 && (int) $user['user_id'] !== (int) $this->current_user_id) : ?>
                                        <a<?= ae('delete_users'); ?> href="#" data-toggle="modal" data-target="#delete-<?= (int) $user['user_id']; ?>"><button type="button" class="btn bg-red"><i class="fa fa-trash"></i></button></a>
                                    <?php endif; ?>
