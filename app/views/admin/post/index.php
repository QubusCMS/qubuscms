...
TriTan\Common\Container::getInstance()->{'set'}('screen_parent', $this->posttype);
TriTan\Common\Container::getInstance()->{'set'}('screen_child', $this->posttype);
                            ...
                            <tr class="gradeX">
                                <td>
                                    <div class="post_title">
                                        <strong><a href="<?= admin_url(sprintf('%s/%s/', $this->posttype, $post->getId())); ?>"><?= $post->getTitle(); ?></a></strong> --
                                        <span class="label <?= ttcms_post_status_label($post->getStatus());?>" style="font-size:1em;font-weight: bold;">
                                            <?= ucfirst($post->getStatus()); ?>
                                        </span>
                                    </div>
                                    <div class="row-actions">
                                        <span class="edit"><a href="<?= admin_url(sprintf('%s/%s/', $this->posttype, $post->getId())); ?>"><?=esc_html__('Edit');?></a></span> |
                                        ...
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-footer">
                                                    ...
                                                    <button type="button" class="btn btn-primary" onclick="window.location='<?= admin_url(sprintf('%s/%s/d/', $this->posttype, $post->getId())); ?>'"><?= esc_html__('Confirm'); ?></button>
                                                </div>
                                            </div>
                                            <!-- /.modal-content -->
                                        </div>
                                ...
                                <td>
                                    <?php if ($post->getAuthor() === get_current_user_id()) : ?>
                                    <a href="<?= admin_url('user/profile/'); ?>"><?= get_name($post->getAuthor(), true); ?></a>
                                    <?php else : ?>
                                    <a href="<?= admin_url(sprintf('user/%s/', $post->getAuthor())); ?>"><?= get_name($post->getAuthor(), true); ?></a>
                                    <?php endif; ?>
                                </td>
                                <td><?=get_user_datetime($post->getPublishedGmt(), 'Y-m-d @ h:i A');?></td>
                                <?php hook::getInstance()->{'doAction'}('manage_post_content_column', 'default', (int) $post->getId());?>
                                <td><?=get_user_datetime($post->getModifiedGmt(), 'Y-m-d @ h:i A');?></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                    <tfoot>
